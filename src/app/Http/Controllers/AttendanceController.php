...
class AttendanceController extends Controller
{
    ...
    public function getAttendance($date = null, Request $request = null)
    {
        ...
        foreach($att_records as $att_record){
            $att_record['working_at'] = '';
            $att_record['closing_at'] = '';
        ...
        //就業時間・休憩時間の取得計算
        foreach($att_records as $att_record){
            $break_records = breaktime::where('user_id', $att_record->id)->where('break_on', $Date)->get();
            
            $work_time = 0;
            if (isset($att_record->closing_at)){
                $work_time = (strtotime($Date . $att_record->closing_at) - strtotime($Date . $att_record->working_at));
            };

            $sum_break_time = 0;
            foreach($break_records as $break_record){
                if(isset($break_record->breakover_at)){
            ...
            if(!($work_time == 0) && !($sum_break_time == 0)){
